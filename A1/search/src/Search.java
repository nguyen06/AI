...
public class Search {
	...
	public String IterativeDeepeningTreeSearch() {
		...
		int limit = 0;
		while(limit++ >= 0){
			String result = DepthLimitTreeSearch(new FrontierFIFO(), limit);
	...
	public String IterativeDeepeningGraphSearch() {
		...
		int limit = 0;
		while(limit++ >= 0){
			String result = DepthLimitedGraphSearch(new FrontierLIFO(), limit);
			if(result != null) return result;
		}
		return null;	
	}
	...
	private String DepthLimitTreeSearch(Frontier frontier, int limit){
		//initialize the frontier using the initia state of problem
		cnt = 0;
		node_list = new ArrayList<Node>();
		...
		while(true)
		{
			...
			if(frontier.isEmpty())
				return null;
			//remove n from the fontier
			Node node = frontier.remove();

			//if n contains a goal state then return the corresponding solution
			if(problem.goal_test(node.state)) return Solution(node);
			// if the depth of n is less than limit 
			// then expand n adding the resulting nodes to the frontier
			if(node.depth < limit) {
				frontier.insertAll(Expand(node,problem));
				cnt++;
			}
	...
	private String DepthLimitedGraphSearch(Frontier frontier,int limit){
		cnt = 0;
		node_list = new ArrayList<Node>();
